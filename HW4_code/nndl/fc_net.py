...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    loss += .5 * self.reg * np.sum(self.params['W1'] * self.params['W1']) + .5*self.reg * np.sum(self.params['W2'] * self.params['W2'])
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(0, self.num_layers):
      ...
      if use_batchnorm and i != (self.num_layers-1):
        self.params['gamma' + str(i+1)] = np.ones(modif_hidden_dims[i])
        self.params['beta' + str(i+1)] = np.zeros(modif_hidden_dims[i])
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers+1):
      ...
      if i == self.num_layers:
        ...
        if self.use_batchnorm:
          scores, self.batchnorm_cache['batchnorm'+id_str] = batchnorm_forward(scores, self.params['gamma' + id_str], self.params['beta'+id_str], self.bn_params[i-1])
        
        scores, cache = affine_relu_forward(scores, self.params[W_name], self.params[b_name])

        if self.use_dropout:
          scores, self.dropout_cache['dropout' + id_str] = dropout_forward(scores, self.dropout_param)
    ...
    for i in range(self.num_layers, 0, -1):
      ...
      if i == self.num_layers:
        ...
        if self.use_dropout:
          der = dropout_backward(der, self.dropout_cache['dropout' + id_str])

        der, grads[W_name], grads[b_name] = affine_relu_backward(der, self.cache[cache_name])

        if self.use_batchnorm:
          der, grads['gamma' + id_str], grads['beta' + id_str] = batchnorm_backward(der, self.batchnorm_cache['batchnorm' + id_str])
