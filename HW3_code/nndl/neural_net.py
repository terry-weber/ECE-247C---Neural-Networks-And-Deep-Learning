...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    scores = z2 - np.max(z2, axis=1, keepdims=True)
    class_probs = np.exp(scores)
    class_probs /= np.sum(class_probs, axis=1, keepdims=True)
  
    loss = - np.sum(np.log(class_probs[range(N), y])) / 5 + .5 * reg*np.sum(W1*W1) + .5*reg*np.sum(W2*W2)
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
    ...
    dscores = class_probs / N
    dL = np.sum(dscores, axis=0)
    db2 = dL
  ...
  def predict(self, X):
    ...
    for i in np.arange(scores.shape[0]):
      max = np.max(scores[i]) 
      y_pred[i] = np.where(scores[i] == max)[0]
