...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  running_mean = bn_param.get('running_mean', np.zeros(D, dtype=x2.dtype))
  running_var = bn_param.get('running_var', np.zeros(D, dtype=x2.dtype))
  ...
  if mode == 'train':
    ...
    out = xn * gamma + beta
    cache = [eps, mu, var, gamma, beta, x2, xc, xn]
...
def batchnorm_backward(dout, cache):
  ...
  dxn = dout * gamma
  dsigninv = np.sum(dxn * xc, axis=0)
  dsig = dsigninv * -1/(var + eps)
