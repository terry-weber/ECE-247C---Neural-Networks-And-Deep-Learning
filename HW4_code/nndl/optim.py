...
def rmsprop(w, dw, config=None):
  ...
  config['a'] = config['beta'] * config['a'] + (1 - config['beta']) * np.multiply(dw, dw)
  next_w = w - np.multiply(config['learning_rate'] , dw) / (np.sqrt(config['a']) + config['epsilon'])
...
def adam(w, dw, config=None):
  ...
  v_u = config['v'] / (1 - config['beta1']**config['t'])
  a_u = config['a'] / (1-config['beta2']**config['t'])
  next_w = w - (np.multiply(config['learning_rate'] , v_u )) / (np.sqrt(a_u) + config['epsilon'])
